...
public class SparkXmlDemo implements Serializable {
    ...
    private void testXfdf(JavaSparkContext sc){
        ...
        SparkSession session = SparkSession.builder().appName("my app").getOrCreate();

        /*Dataset explosion = session.read()
                .format("com.databricks.spark.xml")
                .option("rowTag", "xfdf")
                .load("transformed-testing-13bf9f4d-d976-4b66-a49f-360e860e02b1.xml");

        Dataset fields = explosion.selectExpr("explode(ids._original) as id");
        fields.select("id.*").show();*/


        Dataset ds = session.read()
                .format("com.databricks.spark.xml")
                .schema(getFieldSchema())
                .option("rootTag", "fields")
                .option("rowTag", "field")
                .option("mode", "DROPMALFORMED")
                .load("transformed-data.xml");
    ...
    private void testBook(JavaSparkContext sc){

        SparkSession session = SparkSession.builder().appName("testMySql app").getOrCreate();

        Dataset dataset_mysql = session.read()
                .format("com.databricks.spark.xml")
    ...
    private static StructType getBookSchema(){
    ...
    private static StructType getFieldSchema(){
